...
#Define fonta and colors
#NONE: Using system defaults

#Define functions
def get_red(slider_value):
    '''Turn current slider value for red into a hex value and update the color box
    The scale value is passed automatically when the scale is moved calling the get red function'''
    global red_value

    #Turn the slider value into an int and make into hex value. Strip leading characters so only two remain
...
def get_green(slider_value):
    '''Turn current slider value for green into a hex value and update the color box
    The scale value is passed automatically when the scale is moved calling the get green function'''
    global green_value
...
def get_blue(slider_value):
    '''Turn current slider value for blue into a hex value and update the color box
    The scale value is passed automatically when the scale is moved calling the get blue function'''
    global blue_value
...
def save_colors():
    '''output the chosen color to a txt file'''
...
#Create button for each complimentary color
...
#Put labels, sliders, and buttons on to the frames...Use ipad with rgb buttons to define column width, then use sticky on others
...
#Put color box and label onto frame
...
#Initialize the dictionary to hold all stored colors
stored_colors = {}
stored_color = IntVar()
...
for i in range(6):
    ...
    #.cget() returns the value of a specific option. Store the text value of the tuple label and hex value
    stored_colors[stored_color.get()] = [new_color_tuple.cget('text'), new_color_hex.cget('text')]
